...
def find_centroid(polygon):
    ...
    for i in range(count-1):
        p = polygon[i]
        p1 = polygon[i+1]
        foo = latitude(p) * longitude(p1)
        bar = latitude(p1) * longitude(p)
        result = foo - bar
        A += result
    ...
    if A==0 or count <= 3 :
        ...
        """Cx=( (latitude(p)+latitude(p1))* (latitude(p)*longitude(p1) - latitude(p1)*longitude(p) )/ (6 * A)
        Cy=( (longitude(p)+longtitude(p1))* (latitude(p)*longitude(p1) - latitude(p1)*longitude(p))/ (6 * A)"""
...
def find_closest_state(tweet, state_centers):
    ...
    for i in state_centers:
        dis= geo_distance(state_centers[i],tweet_location(tweet))
        if dis < short :
            short=dis
            s = i
...
def group_tweets_by_state(tweets):
    ...
    tweets_by_state = {n:[] for n in us_states} 
    #remember that you are making a dictionary whose values will consist of a bunch of lists
    us_centers = {n: find_center(us_states[n]) for n in us_states} 
    
    "*** YOUR CODE HERE ***"
    for tweet in tweets:
        closest = find_closest_state(tweet,us_centers)
...
def most_talkative_state(term):
    ...
    state_most = 0
    for i in state_dict:
        state_list = state_dict[i]
        if len(state_list)>tweet_most:
            tweet_most = len(state_list)
            state_most = str(i)
    return state_most
...
def average_sentiments(tweets_by_state):
    ...
    for i in tweets_by_state:
        total =0.0
        count =0
        for tweet in tweets_by_state[i]:
            sentiment = analyze_tweet_sentiment(tweet)
            if sentiment != None:
                total += sentiment
                count += 1
            if count != 0:
                averaged_state_sentiments[i] = total/count

...
class World:
    def __init__(self, data):
        ''' constructor: called with a parsed TMX file '''
        self.data = data
        self.x = 0
        self.y = 0
        self.sprites = {}
    ...
    def addSprite(self, sprite):
        ''' add a sprite to the world (and the collision matrix) '''
        if sprite.name not in self.sprites:
            self.sprites[sprite.name] = sprite
            self.addToCollisionMatrix(sprite)

    def removeSprite(self, sprite):
        ''' remove a sprite from the world (and the collision matrix) '''
    ...
    def addToCollisionMatrix(self, sprite):
        ''' add a sprite to the collision matrix '''
        if sprite.name not in self.sprites:
            return
    ...
    def removeFromCollisionMatrix(self, sprite):
        ''' remove a sprite from the collision matrix '''
        (sizex, sizey) = (self.data.width, self.data.height)
        (tilesizex, tilesizey) = (self.data.tilewidth, self.data.tileheight)
        for (x, y) in sprite.getPoints():
            xx, yy = int(x) / tilesizex, int(y) / tilesizey
    ...
    def findCollisions(self, sprite):
        ''' find any collisions with the given sprite '''
        (sizex, sizey) = (self.data.width, self.data.height)
        (tilesizex, tilesizey) = (self.data.tilewidth, self.data.tileheight)
        collisions = {}
        candidates = {}
    ...
    def paint(self, surface):
        ''' draw a portion of the background based on a top-left corner position '''
        # blank the screen
        ...
        viewsize_y = (surface.get_width() + tilesizey - 1) / tilesizey
